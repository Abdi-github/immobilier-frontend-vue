...
vi.mock('@/shared/composables/useLocalizedName', () => ({
  useLocalizedName: () => (name: Record<string, string>) => name?.en ?? '',
}));

vi.mock('@/shared/utils/formatters', () => ({
  formatPrice: (price: number, currency?: string) => `${currency ?? 'CHF'} ${price?.toLocaleString() ?? '0'}`,
}));
...
const mountCard = (overrides: Partial<Property> = {}, isFavorite = false) =>
  mount(PropertyCardGrid, {
